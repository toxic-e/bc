...
// ── TELEGRAM DIRECT NOTIFICATION (works on Vercel serverless — no polling needed) ──
async function notifyTelegram(order, uploadedFilePath, uploadedFileBuffer) {
  ...
  // 2. Send screenshot for free orders
  if (isFree) {
    const photoBuffer = uploadedFileBuffer
      || (uploadedFilePath ? fs.readFileSync(uploadedFilePath) : null);
    if (photoBuffer) {
      const fd = new FormData();
      fd.append('chat_id', String(chatId));
      fd.append('caption', `📸 YT Subscribe Proof — Order \`${order.orderId}\``);
      fd.append('parse_mode', 'Markdown');
      fd.append('photo', new Blob([photoBuffer], { type: 'image/jpeg' }), 'screenshot.jpg');
...
app.post('/api/orders', orderLimiter, upload.single('screenshot'), async (req, res) => {
  ...
  try {
    ...
    // NOTE: On Vercel (memoryStorage), file is in uploadedFileBuffer — not uploadedFilePath
    if (isFree && !uploadedFilePath && !uploadedFileBuffer) {
    ...
    // Notify Telegram via direct HTTP (works on Vercel + local, no polling needed)
    notifyTelegram(order, uploadedFilePath, uploadedFileBuffer).catch(console.error);
